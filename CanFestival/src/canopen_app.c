...
//	eprintf("TestMaster_post_TPDO\n");
	printf("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxTestMaster_post_TPDO\n");
	SDK_DATA_B0++;
}
...
void canopen_app_init(int argc,char **argv)
{
	int c;
  extern char *optarg;
...
pthread_t canopen_app_thread_flag;
void signal_func(void)
{
  printf("canopen_app_thread\r\n");
  StopTimerLoop(&Exit);
  canClose(&TestMaster_Data);
  pthread_exit(NULL);
  pthread_cancel(canopen_app_thread_flag);
}

void* canopen_app_thread(void* ptr)
{
...
void canopen_app_thread_create(void)
{
	int temp;
	memset(&canopen_app_thread_flag,0,sizeof(canopen_app_thread_flag));
	if((temp = pthread_create(&canopen_app_thread_flag,NULL,canopen_app_thread,NULL))!=0)
	{
		printf("canopen_app_thread create ERROR!\r\n");
	}
	else
	{
		printf("canopen_app_thread create success!\r\n");
	}
}

void canopen_app_thread_wait(void)
{
	if(canopen_app_thread_flag != 0)
	{
		pthread_join(canopen_app_thread_flag,NULL);
		printf("anopen_app_thread end!\r\n");
	}
}
...
//
//	StartTimerLoop(&InitNodes);
//	printf("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxSDK_DATA_B0 = %x\n",SDK_DATA_B0);
