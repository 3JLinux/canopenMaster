...
{
  UNS16 offset = d->firstIndex->SDO_SVR;		// 获取SDO 服务器索引
...
#endif
  if(!(nodeId>0 && nodeId<=127)){				// 判断ID是否有效 Node-ID 有效范围为 1-127
	  MSG_WAR(0x2D01, "Invalid NodeID",nodeId);
	  return;
  }

  if(offset){
    /* Adjust COB-ID Client->Server (rx) only id already set to default value or id not valid (id==0xFF)*/ // 服务器ID设置为无效值或设置为默认值
    if((*(UNS32*)d->objdict[offset].pSubindex[1].pObject == 0x600 + *d->bDeviceNodeId)||(*d->bDeviceNodeId==0xFF)){  // 若服务器ID设置为默认值
      /* cob_id_client = 0x600 + nodeId; */
      *(UNS32*)d->objdict[offset].pSubindex[1].pObject = 0x600 + nodeId;	// 设置 ID 接收端
    }
    ...
    if((*(UNS32*)d->objdict[offset].pSubindex[2].pObject == 0x580 + *d->bDeviceNodeId)||(*d->bDeviceNodeId==0xFF)){
      ...
      *(UNS32*)d->objdict[offset].pSubindex[2].pObject = 0x580 + nodeId;	// 设置 ID 发送端
    }
  }
