...
{
  const indextable *TPDO_com = d->objdict + d->firstIndex->PDO_TRS + numPdo; // 初始地址+偏移量(起始索引号+nodeid)
  const indextable *TPDO_map = d->objdict + d->firstIndex->PDO_TRS_MAP + numPdo;	// 映射初始地址+偏移量(起始索引号+nodeid)

  UNS8 prp_j = 0x00;
  UNS32 offset = 0x00000000;
  const UNS8 *pMappingCount = (UNS8 *) TPDO_map->pSubindex[0].pObject;				// 获取映射数目

  pdo->cob_id = (UNS16) UNS16_LE(*(UNS32*)TPDO_com->pSubindex[1].pObject & 0x7FF);	// 获取子索引号为1中设定的ID值
  pdo->rtr = NOT_A_REQUEST;

  MSG_WAR (0x3009, "  PDO CobId is : ",					//testdeleteyzh
  ...
  do
    {
      ...
      UNS32 *pMappingParameter =
        (UNS32 *) TPDO_map->pSubindex[prp_j + 1].pObject;	// 映射中的值
      ...
      if (Size && ((offset + Size) <= 64)) // 映射中索引与子索引值长度小于64位
        {
          UNS32 ByteSize = 1 + ((Size - 1) >> 3);        /*1->8 => 1 ; 9->16 => 2, ... */
          UNS8 subIndex =
            (UNS8) (((*pMappingParameter) >> (UNS8) 8) & (UNS32) 0x000000FF);

          MSG_WAR (0x300F, "  got mapping parameter : ", *pMappingParameter);		//testdeleteyzh
  ...
  pdo->len = (UNS8)(1 + ((offset - 1) >> 3)); //将长度传给len

  MSG_WAR (0x3015, "  End scan mapped variable", 0);	//testdeleteyzh
        ...
        while (offsetObjdict <= lastIndex)
          {
            switch (status)
              ...
              case state2:
                pwCobId = d->objdict[offsetObjdict].pSubindex[1].pObject;	// 取ID
                if (*pwCobId == UNS16_LE(m->cob_id))						// 判断ID是否一致
                  {
              ...
              case state4:     /* Get Mapped Objects Number */
                /* The cobId of the message received has been found in the
                   dictionnary. */
                offsetObjdict = d->firstIndex->PDO_RCV_MAP;				// 获取映射首个MAP偏移量
                lastIndex = d->lastIndex->PDO_RCV_MAP;					// 获取映射最后一个MAP偏移量
                pMappingCount =
                  (UNS8 *) (d->objdict + offsetObjdict +
                            numPdo)->pSubindex[0].pObject;
                numMap = 0;
                while (numMap < *pMappingCount)							// 获取映射map中子索引个数
                  {
                    UNS8 tmp[] = { 0, 0, 0, 0, 0, 0, 0, 0 };
                    UNS32 ByteSize;
                    pMappingParameter =									// 子索引中的数据
                    ...
                    Size = (UNS8) (*pMappingParameter & (UNS32) 0x000000FF);	// 获取子索引数据长度
                    ...
                    if (Size && ((offset + Size) <= (m->len << 3)))
                      {
                        ...
                        objDict =
                          setODentry (d, (UNS16) ((*pMappingParameter) >> 16),	// 获取数据存储映射地址（2000后）
                                      (UNS8) (((*pMappingParameter) >> 8) &		// 获取子索引标号 即第几个参数
                                              0xFF), tmp, &ByteSize, 0);
...
static void sendPdo(CO_Data * d, UNS32 pdoNum, Message * pdo)
{
  ...
  MSG_WAR (0x396D, "sendPDO cobId :", UNS16_LE(pdo->cob_id));	//testdeleteyzh
...
sendOnePDOevent (CO_Data * d, UNS8 pdoNum)
{
  ...
  MSG_WAR (0x3968, "  PDO is on EVENT. Trans type : ",					//testdeleteyzh
           *((UNS8 *) d->objdict[offsetObjdict].pSubindex[2].pObject));
  ...
  if (d->PDO_status[pdoNum].last_message.cob_id == pdo.cob_id
  ...
  else
    {
      ...
      MSG_WAR (0x306A, "Changes TPDO number : ", pdoNum);	//testdeleteyzh
...
{
  UNS8 pdoNum = 0x00;           /* number of the actual processed pdo-nr. */  // 设备Node-ID
  UNS8 *pTransmissionType = NULL;
  UNS8 status = state3;
  UNS16 offsetObjdict = d->firstIndex->PDO_TRS;								// TPDO的索引(起始)
  UNS16 offsetObjdictMap = d->firstIndex->PDO_TRS_MAP;						// TPDO映像的索引(起始)
  UNS16 lastIndex = d->lastIndex->PDO_TRS;									// TPDO的索引(结束)

  if (!d->CurrentCommunicationState.csPDO)									// 判断当前通信状态
    {
      return 0;
    }
  ...
  if (offsetObjdict)														// 判断是否有PDO模式
    {
      Message pdo;/* = Message_Initializer;*/
      memset(&pdo, 0, sizeof(pdo));
      while (offsetObjdict <= lastIndex)									// 遍历所有PDO数据
        {
          switch (status)
            {
            case state3:
              ...
              pTransmissionType =
                (UNS8 *) d->objdict[offsetObjdict].pSubindex[2].pObject;	// 获取子索引为02即发送类型值
              MSG_WAR (0x3962, "Reading PDO at index : ", 0x1800 + pdoNum);	// testdeleteyzh

              /* check if transmission type is SYNCRONOUS */
              /* message transmited every n SYNC with n=TransmissionType */
              if (isSyncEvent &&
                  (*pTransmissionType >= TRANS_SYNC_MIN) &&					// 若发送类型为同步类型
                  (*pTransmissionType <= TRANS_SYNC_MAX) &&
                  (++d->PDO_status[pdoNum].transmit_type_parameter ==		// 接收到*pTransmissionType个同步帧后进入该判断
                   *pTransmissionType))
                {
                  /*Reset count of SYNC */
                  d->PDO_status[pdoNum].transmit_type_parameter = 0;		// 将同步接收次数清零
            ...
            case state11:      /*Go to next TPDO */
              ...
              offsetObjdict++;
              offsetObjdictMap++;
              MSG_WAR (0x3970, "next pdo index : ", pdoNum);	//testdeleteyzh
              status = state3;
              break;
