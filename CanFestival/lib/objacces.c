...
{ /* DO NOT USE MSG_ERR because the macro may send a PDO -> infinite
  ...
  if (errorCode != OD_SUCCESSFUL)
    return errorCode;
  if( ptrTable->bSubCount <= bSubindex ) {								// 想要获取的子索引大于索引个数
    /* Subindex not found */
    accessDictionaryError(wIndex, bSubindex, 0, 0, OD_NO_SUCH_SUBINDEX);
    return OD_NO_SUCH_SUBINDEX;
  }

  if (checkAccess && (ptrTable->pSubindex[bSubindex].bAccessType & WO)) {	// 检查数据类型是否可读，若不可读则返回不可读
  ...
  if (pDestData == 0) {													// 数据存放地址是否为空
    return SDOABT_GENERAL_ERROR;
  }

  if (ptrTable->pSubindex[bSubindex].size > *pExpectedSize) {			// 想要读取子索引字节数小于对象列表中的字节数
  ...
  *pDataType = ptrTable->pSubindex[bSubindex].bDataType;				// 赋值 数据类型
  szData = ptrTable->pSubindex[bSubindex].size;							// 赋值 字节数
...
#  endif
  ...
  if(*pDataType != visible_string) {								// 数据类型不是字符串类型
                   ...
                   UNS8 endianize)
{
  ...
  if( ptrTable->bSubCount <= bSubindex ) {												// 想获取的索引大于自索引个数
    /* Subindex not found */
    accessDictionaryError(wIndex, bSubindex, 0, *pExpectedSize, OD_NO_SUCH_SUBINDEX);
    return OD_NO_SUCH_SUBINDEX;
  }
  if (checkAccess && (ptrTable->pSubindex[bSubindex].bAccessType == RO)) {				// 判断对象是否为只读，若为只读则返回
  ...
  dataType = ptrTable->pSubindex[bSubindex].bDataType;									// 赋值参数类型
  szData = ptrTable->pSubindex[bSubindex].size;											// 赋值参数大小

  if( *pExpectedSize == 0 ||															// 判断是写入字节是否为零或与索引字节一样
      *pExpectedSize == szData ||
      /* allow to store a shorter string than entry size */
      (dataType == visible_string && *pExpectedSize < szData))							// 判断数据类型是否为字符串且写入字节数小于索引内字节数
    {
#ifdef CANOPEN_BIG_ENDIAN
...
UNS32 RegisterSetODentryCallBack(CO_Data* d, UNS16 wIndex, UNS8 bSubindex, ODCallback_t Callback) // 注册写字典回调函数
{
UNS32 errorCode;
