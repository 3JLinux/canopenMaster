...
struct struct_CO_Data {
	/* Object dictionary */
	UNS8 *bDeviceNodeId;					// 记录Node-ID
	const indextable *objdict;				// 存放每个对象的索引，可通过索引找到存放对象的位置
	s_PDO_status *PDO_status;				// 保存发送PDO通信参数的数组
	TIMER_HANDLE *RxPDO_EventTimers;		//
	void (*RxPDO_EventTimers_Handler)(CO_Data*, UNS32);
	const quick_index *firstIndex;			// 用于存储SDO PDO 对象字典中的索引值 	起始
	const quick_index *lastIndex;			// 用于存储SDO PDO 对象字典中的索引值 	结束
	const UNS16 *ObjdictSize;				// 对象字典里面对象的个数
	const UNS8 *iam_a_slave;				// 主机或从机标识 0：主机，1：从机
	valueRangeTest_t valueRangeTest;		// 函数指针，用于检测值是否的超出范围
	
	/* SDO */
	s_transfer transfers[SDO_MAX_SIMULTANEOUS_TRANSFERS];	// SDO传输的数据结构
	/* s_sdo_parameter *sdo_parameters; */

	/* State machine */
	e_nodeState nodeState;									// 设备状态  初始化、预操作、操作、停止
	s_state_communication CurrentCommunicationState;		// 当前通讯状态
	initialisation_t initialisation;						// 初始化函数指针	can通讯超载时使用
	preOperational_t preOperational;						// 预操作函数指针	can通讯超载时使用
	operational_t operational;								// 操作函数指针		can通讯超载时使用
	stopped_t stopped;										// 停止函数指针		can通讯超载时使用
     void (*NMT_Slave_Node_Reset_Callback)(CO_Data*);
     void (*NMT_Slave_Communications_Reset_Callback)(CO_Data*);
     
	/* NMT-heartbeat */
	UNS8 *ConsumerHeartbeatCount;							// 消费者 心跳计数
	UNS32 *ConsumerHeartbeatEntries;						// 消费者心跳进入
	TIMER_HANDLE *ConsumerHeartBeatTimers;					// 消费者心跳回调任务执行状态
	UNS16 *ProducerHeartBeatTime;							// 生产者心跳时间
	TIMER_HANDLE ProducerHeartBeatTimer;					// 生产者心跳回调任务执行状态
	heartbeatError_t heartbeatError;
	e_nodeState NMTable[NMT_MAX_NODE_ID]; 					// 节点状态
	...
	UNS32 *COB_ID_Sync;										// 同步的ID值 通常为0x080
