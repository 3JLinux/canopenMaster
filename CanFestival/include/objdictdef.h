...
{
    UNS8                    bAccessType;	// 读写类型	RO:只读，RW：读写，WO：只写
    UNS8                    bDataType; /* Defines of what datatype the entry is */	// 存储参数类型
    UNS32                   size;      /* The size (in Byte) of the variable */		// 数据大小
    void*                   pObject;   /* This is the pointer of the Variable */	//表示这个对象保存数据变量的指针，即数据真正存储的位置 - 对象字典中的值
} subindex;
...
{
    subindex*   pSubindex;   /* Pointer to the subindex */		//保存子索引相关信息
    UNS8   bSubCount;   /* the count of valid entries for this subindex	//子索引个数
...
typedef UNS32 (*ODCallback_t)(CO_Data* d, const indextable *, UNS8 bSubindex);	// 原型为(UNS32)(*xx)(CO_Data* d, const indextable *, UNS8 bSubindex) 定义为一个函数指针  写入事件时的回调函数
typedef const indextable * (*scanIndexOD_t)(UNS16 wIndex, UNS32 * errorCode, ODCallback_t **Callback); //指向一个常量函数指针的indextable类型指针.
