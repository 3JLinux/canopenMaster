...
struct struct_s_PDO_status {
  UNS8 transmit_type_parameter;		// PDO的通信类型，有同步、异步、周期、非周期等
  TIMER_HANDLE event_timer;			// 若为定时发送则此参数保存定时时间，若为0则为事件类型发送
  TIMER_HANDLE inhibit_timer;		// PDO发送的最小时间间隔
  Message last_message;				// 保存最新的TPDO message
};
 ...
 * type and content change before sending it.	函数迭代所有的TPDO 在发送前查看发送类型和内容
 * @param *d Pointer on a CAN object data structure
 * @param isSyncEvent
 */
